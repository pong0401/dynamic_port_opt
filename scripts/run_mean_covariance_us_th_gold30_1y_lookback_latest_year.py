...
from run_mean_covariance_penalty_sweep import _concentration_stats  # noqa: E402
from run_mean_covariance_us_th_overlay_gold30 import (  # noqa: E402
    _best_signal_config,
...
STRATEGY = "US+TH Mean Covariance Gold30 stock cap 8 mom_63 1Y lookback latest-year rebalance"
...
def _latest_backtest_start(index: pd.DatetimeIndex) -> pd.Timestamp:
    end = pd.Timestamp(index.max())
    return pd.Timestamp(index[max(0, len(index) - BACKTEST_DAYS - 1)]).normalize() if len(index) > BACKTEST_DAYS else pd.Timestamp(index.min())


def run_backtest(latest_year_only: bool = True, strategy_name: str = STRATEGY) -> dict[str, object]:
    paths = default_paths(ROOT)
    ...
    schedule_all = monthly_rebalance_dates(prices.index, lookback_days=LOOKBACK_DAYS, freq="ME")
    if latest_year_only:
        latest_start = _latest_backtest_start(prices.index)
        schedule = [date for date in schedule_all if date >= latest_start]
    else:
        schedule = list(schedule_all)
    if len(schedule) < 2:
        raise RuntimeError("Not enough rebalance dates were available.")

    nav = pd.Series(1.0, index=[schedule[0]], name=strategy_name)
    ...
    for idx, rebalance_date in enumerate(schedule[:-1]):
        ...
        th_selected = select_point_in_time_universe(
            prices.reindex(train_index),
            volumes.reindex(train_index),
            th_pool,
            n_assets=TH_ASSETS,
    ...
    metrics["Backtest Start"] = nav.index.min().date().isoformat()
    metrics["Backtest End"] = nav.index.max().date().isoformat()
    metrics["Rebalance Count"] = len(weights_history)
    return {
        "nav": nav,
        "metrics": metrics,
...
def apply_asset_level_daily_exposure(results: dict[str, object]) -> dict[str, object]:
    ...
    nav = (1.0 + exposed_returns).cumprod().rename(DAILY_STRATEGY)
    metrics = _metrics_for_nav(nav, benchmark)
...
def _latest_weights_frame(history: dict[pd.Timestamp, pd.Series], strategy: str, weight_col: str) -> pd.DataFrame:
    if not history:
        return pd.DataFrame()
    latest_date = max(history)
    latest = history[latest_date].rename(weight_col).reset_index()
    latest.columns = ["Asset", weight_col]
    ...
    return latest.sort_values(weight_col, ascending=False)


def _latest_effective_frame(effective_weights: pd.DataFrame) -> pd.DataFrame:
...
def _sleeve_history(weights: pd.DataFrame) -> pd.DataFrame:
    if weights.empty:
        return pd.DataFrame()
    frame = pd.DataFrame(index=weights.index)
    stock_cols = [column for column in weights.columns if column not in OVERLAY_ASSETS and column != "Cash / Reduced Exposure"]
    us_cols = [column for column in stock_cols if not str(column).endswith(".BK")]
    th_cols = [column for column in stock_cols if str(column).endswith(".BK")]
    frame["US Equity"] = weights.reindex(columns=us_cols).sum(axis=1) if us_cols else 0.0
    frame["TH Equity"] = weights.reindex(columns=th_cols).sum(axis=1) if th_cols else 0.0
    for asset, sleeve in {"GC=F": "Gold", "BTC-USD": "BTC", "BIL": "BIL", "Cash / Reduced Exposure": "Cash"}.items():
        if asset in weights.columns:
            frame[sleeve] = weights[asset]
    return frame
...
def main() -> None:
    ...
    summary = pd.DataFrame([raw_row, daily_row])
    for column, value in {
        "US Assets": US_ASSETS,
        "TH Assets": TH_ASSETS,
    ...
    latest_effective = daily["effective_weights"].drop(columns=["Cash / Reduced Exposure"], errors="ignore").iloc[-1]
    for key, value in _concentration_stats(latest_effective).items():
        summary.loc[summary["Strategy"].eq(DAILY_STRATEGY), f"Latest {key}"] = value
    summary = summary.sort_values("Sharpe", ascending=False)

    result_prefix = "mean_covariance_us_th_gold30_stockcap8_1y_lookback_latest_year"
    summary.to_csv(paths.result_dir / f"{result_prefix}_summary.csv", index=False)
    pd.DataFrame({"Raw": raw["nav"], "Asset Daily Exposure": daily["nav"]}).dropna(how="all").to_csv(
        paths.result_dir / f"{result_prefix}_curves.csv"
    )
    _latest_weights_frame(raw["weights_history"], STRATEGY, "Portfolio Weight").to_csv(
        paths.result_dir / f"{result_prefix}_latest_weights.csv",
        index=False,
    )
    daily["exposure_history"].to_csv(paths.result_dir / f"{result_prefix}_daily_exposure_history.csv")
    daily["effective_weights"].to_csv(paths.result_dir / f"{result_prefix}_daily_effective_weights.csv")
    _latest_effective_frame(daily["effective_weights"]).to_csv(
        paths.result_dir / f"{result_prefix}_latest_effective_weights.csv",
        index=False,
    )
    _sleeve_history(daily["effective_weights"]).to_csv(paths.result_dir / f"{result_prefix}_sleeve_weight_history.csv")

    print(summary.to_string(index=False, float_format=lambda value: f"{value:.6f}"))
    latest = _latest_effective_frame(daily["effective_weights"])
