...
GROUP_CAP_OUTPUT_PREFIX = f"{OUTPUT_PREFIX}_group_cap_sweep"
CASH_ASSET = "Cash / Reduced Exposure"
GROUP_CAPS: list[float | None] = [None, 0.40, 0.50, 0.60]
...
def _optimize_with_group_caps(
    ...
    if group_cap is None:
        from dynamic_factor_copula import optimize_portfolio
        ...
        ).reindex(base_assets).fillna(0.0)

    cash_cap = max(0.0, 1.0 - (group_cap + (group_cap if th_is_on else 0.0) + GOLD_CAP + BTC_CAP))
    ...
    caps.loc[[asset for asset in assets if asset == "GC=F"]] = GOLD_CAP
    caps.loc[[asset for asset in assets if asset == "BTC-USD"]] = BTC_CAP
    ...
    if us_assets:
        ...
        constraints.append({"type": "ineq", "fun": lambda x, idx=us_idx: group_cap - float(np.sum(x[idx]))})
    if th_assets:
        th_idx = [assets.index(asset) for asset in th_assets if asset in assets]
        constraints.append({"type": "ineq", "fun": lambda x, idx=th_idx: group_cap - float(np.sum(x[idx]))})
    ...
    if not result.success:
        weights = pd.Series(x0, index=assets)
    else:
...
def _run_case(group_cap: float | None) -> tuple[pd.Series, pd.Series, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    label = "No group cap" if group_cap is None else f"US/TH group cap {group_cap:.0%}"
    ...
    for idx, rebalance_date in enumerate(schedule[:-1]):
        ...
        weights = _optimize_with_group_caps(cov, momentum_signal, current_us, current_th, group_cap, th_is_on)
...
def main() -> None:
    ...
    for group_cap in GROUP_CAPS:
        raw_curve, exposed_curve, raw_weights, effective_weights, selected = _run_case(group_cap)
        case = "No group cap" if group_cap is None else f"US/TH group cap {group_cap:.0%}"
        all_curves[raw_curve.name] = raw_curve
        all_curves[exposed_curve.name] = exposed_curve
        ...
        for strategy, curve, weights, mode in [
            ...
            row.update({"Case": case, "Group Cap": group_cap, "Mode": mode, "Stock Cap": STOCK_CAP, "Gold Cap": GOLD_CAP, "BTC Cap": BTC_CAP})
    ...
    summary = pd.DataFrame(all_summary).sort_values(["Sharpe", "CAGR"], ascending=False)
    selected_history = pd.concat(all_selected, ignore_index=True)
    ...
    for case, frame in effective_history.groupby("Case"):
        ...
        grouped_frames.append(grouped.reset_index(names="Date"))
    grouped_history = pd.concat(grouped_frames, ignore_index=True)
    summary.to_csv(paths.result_dir / f"{GROUP_CAP_OUTPUT_PREFIX}_summary_thb.csv", index=False)
    curves.to_csv(paths.result_dir / f"{GROUP_CAP_OUTPUT_PREFIX}_curves_thb.csv")
    selected_history.to_csv(paths.result_dir / f"{GROUP_CAP_OUTPUT_PREFIX}_universe_history_thb.csv", index=False)
    latest_weights.to_csv(paths.result_dir / f"{GROUP_CAP_OUTPUT_PREFIX}_latest_weights_thb.csv", index=False)
    effective_history.to_csv(paths.result_dir / f"{GROUP_CAP_OUTPUT_PREFIX}_effective_weights_thb.csv", index=False)
    grouped_history.to_csv(paths.result_dir / f"{GROUP_CAP_OUTPUT_PREFIX}_grouped_weight_history_thb.csv", index=False)
    _period_compare(curves).to_csv(paths.result_dir / f"{GROUP_CAP_OUTPUT_PREFIX}_period_compare_thb.csv", index=False)
    cols = [
        "Strategy",
        "CAGR",
        "Annual Vol",
    ...
    print(summary[cols].head(12).to_string(index=False, float_format=lambda value: f"{value:.4f}"))
