...
OUTPUT_PREFIX = "spy_gold_btc_bil_balanced_country8_refine"
...
def main() -> None:
    ...
    gold_rules = []
    for warn_dd in [-0.06, -0.08, -0.10]:
        for crash_dd in [-0.18, -0.20, -0.22]:
            if crash_dd >= warn_dd:
                continue
            gold_rules.append(
                adaptive.GoldRule(
                    f"dd252_warn{abs(warn_dd):.0%}_crash{abs(crash_dd):.0%}_half",
                    "dd_simple",
                    warn_dd=warn_dd,
                    crash_dd=crash_dd,
                    warn_exposure=0.50,
    ...
    for rule in gold_rules:
        for trigger in ["spy_below_ma200_or_dd8", "spy_below_ma300_or_dd10"]:
            for boost in [0.14, 0.15, 0.16]:
                name = f"country8_refine {rule.name} boost{boost:.0%}_{trigger}"
                configs.append(refine.Config(name, rule, trigger, boost, 0.08, 2))
